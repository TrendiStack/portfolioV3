...
const Mockup = ({ image, gif }) => {
  const { modal, gifState, handleModal, setGifState } =
  ...
  return (
    <>
      ...
      >
        ...
        >
          <HiArrowLeft
            onClick={() => setGifState(false)}
            className="absolute top-[50%] left-[5%] lg:left-10 transform -translate-y-1/2 text-white text-4xl"
          />
          {!gifState ? (
            <img
              src={image}
              alt=""
              className="w-[60%] mt-5 disable-highlight"
            />
          ) : (
            <img src={gif} alt="" className="w-[60%] mt-5 disable-highlight" />
          )}
          ...
          <HiArrowRight
            onClick={() => setGifState(true)}
            className="absolute top-[50%] right-[5%] lg:right-10 transform -translate-y-1/2 text-white text-4xl"
          />
        </div>
      </div>
      <div
